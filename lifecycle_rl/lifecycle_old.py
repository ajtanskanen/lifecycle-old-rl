...
class Lifecycle():
    ...
    def explain(self):
        ...
        print('perustulo {}\nkarenssi_kesto {}\nmortality {}\nrandomness {}\n'.format(self.perustulo,self.karenssi_kesto,self.mortality,self.randomness))
    ...
    def get_multiprocess_env(self,rlmodel,save,start_from=None,debug=False,add_dir=True,dir='saved'):
        ...
        if rlmodel=='a2c':
            ...
            n_cpu = 4
            savename = "a2c_"+save
            loadname = "a2c_"+start_from
        elif rlmodel=='acer':
            policy_kwargs = dict(act_fun=tf.nn.relu, net_arch=[64, 64, 16])
            n_cpu = 4
            savename = "acer_"+save
            loadname = "acer_"+start_from
        elif rlmodel=='acktr':
            policy_kwargs = dict(act_fun=tf.nn.relu, net_arch=[512, 512]) # 256, 256?
            n_cpu = 12
            savename = "acktr_"+save
            loadname = "acktr_"+start_from
        elif rlmodel=='lstm':
            policy_kwargs = dict()
            n_cpu = 4
            savename = "acktr_"+save
            ...
            n_cpu = 4
            savename = "trpo_"+save
            loadname = "trpo_"+start_from
        else:
            policy_kwargs = dict()
            n_cpu = 1
            savename = "dqn_"+save
            loadname = "dqn_"+start_from   
    ...
    def make_env(self,env_id, rank, kwargs, seed=None):
        """
    ...
    def train(self,train=False,debug=False,steps=20000,cont=False,rlmodel='dqn',\
                ...
                start_from=None,add_dir=True,dir='saved',max_n_cpu=100):
        ...
        policy_kwargs,n_cpu,savename,loadname=self.get_multiprocess_env(self.rlmodel,save,\
                                                    start_from,debug=debug,add_dir=add_dir,dir=dir)  
    ...
    def simulate(self,debug=False,rlmodel=None,plot=True,load=None,pop=None,\
                 ...
                 deterministic=False,save='simulate',add_dir=True,dir='saved'):
    ...
    def test_RL(self,train=False,debug=False,steps=20000,cont=False,rlmodel='dqn',\
                save='unemp',pop=None,batch=2,max_grad_norm=0.5,learning_rate=0.25,\
                start_from=None,perustulo=False):
        from stable_baselines.common.vec_env import SubprocVecEnv,DummyVecEnv
        from stable_baselines import A2C, ACER, DQN, ACKTR #, TRPO
        ...
        policy_kwargs,n_cpu,savename,loadname=self.get_multiprocess_env(rlmodel,save,start_from,add_dir=add_dir,dir=dir)
        ...
        if train:
            model=self.setup_rlmodel(rlmodel,loadname,env,batch,policy_kwargs,learning_rate,max_grad_norm,cont)
            print('training...')
            model.learn(total_timesteps=steps)
            model.save(savename)
            print('done')
        ...
        while n<self.n_pop:
            act, predstate = model.predict(states)
            newstate, rewards, dones, info = env.step(act)
    ...
    def run_results(self,n=2,steps1=100,steps2=100,pop=1_000,rlmodel='acktr',\
               ...
               train=True,predict=True,batch=1):
    ...
    def compare_simstats(self,filename1,filename2):
        ...
        m_best1,m_emp1,m_meadian1,s_emp1=self.get_simstats(filename1)
        m_best2,m_emp2,m_meadian2,s_emp2=self.get_simstats(filename2)
